...
archived release:
\href{https://doi.org/10.5281/zenodo.20961001}{https://doi.org/10.5281/zenodo.20961001}.}
\end{abstract}
...
\href{https://github.com/ShengyaoLiang/csmt-gnn}{https://github.com/ShengyaoLiang/csmt-gnn}.}
The current public release line is \texttt{v0.1.3}.  Archived releases are
...
the archived \texttt{v0.1.3} release is
\href{https://doi.org/10.5281/zenodo.20961001}{https://doi.org/10.5281/zenodo.20961001}.
...
In the current repository I operationalize this with a static dependency
preservation checker for generated candidates: required imports, local
definition bindings, and guarded attribute accesses are counted directly, and
the reported quantity is preserved dependencies divided by required
dependencies.  This metric is still narrow, but it tests the failure mode more
directly than loss alone.
...
\begin{itemize}
  ...
  \item \texttt{scripts/structural\_hallucination\_eval.py}: direct dependency-preservation check for reference or generated snippets.
  \item \texttt{scripts/block\_size\_sensitivity.py}: block-size sensitivity diagnostic at \texttt{B=32,64,128}.
...
unit tests now pass at 41/41 after adding fail-fast checks for the Transformer
...
diagnostic cases.  The direct structural-dependency checker preserves 6 of 6
required dependencies on the reference snippets, with parse success rate 1.0;
this is only a sanity check because no generated candidates are bundled with
the release.  The CVD audit recorded 72 variable-eligible blocks and 112
random-control eligible blocks over 24 audited steps, with both sampling rates
near the configured 0.2.  A five-seed CVD control showed that random block
replacement and variable-targeted CVD are numerically almost identical in this
tiny setting.  I treat this record only as a mechanism and plumbing check: it
shows that the reference paths can train on CPU and that the masks are
observable, not that the architecture is better than a baseline.
...
\begin{table}[t]
  \centering
  \caption{Seed 1--5 mean local diagnostic results at block size 8.  Parentheses
  ...
  \begin{tabular}{lrrr}
    ...
    \midrule
    \texttt{transformer\_baseline} & 4688 & 3.7466 (0.0664) & 3.7939 (0.1964) \\
    \texttt{transformer\_matched} & 6992 & 3.7029 (0.0673) & 3.5655 (0.3241) \\
    \texttt{ast\_graph} & 6960 & 3.6608 (0.0836) & 3.5089 (0.2441) \\
    \texttt{variable\_cvd} & 6976 & 3.7325 (0.0610) & 3.8846 (0.2214) \\
    \texttt{full\_moe} & 8160 & 3.7526 (0.0994) & 3.6296 (0.1806) \\
    \bottomrule
  \end{tabular}
\end{table}
...
seeds 1--5, \texttt{ast\_graph} has the lowest mean cross-block use loss in this
tiny diagnostic, but the rough parameter-neighbor Transformer is close and has a
large standard deviation.  A separate seed 1--5 control gives
\texttt{random\_dropout\_control} and \texttt{variable\_cvd} the same rounded
final loss, eval loss, and cross-block use loss; the mean variable-minus-random
final-loss delta is only $-2.8\times 10^{-6}$.  This is exactly the kind of
result I wanted the diagnostic to be able to show: the architecture is now
comparable against a real Transformer control, but it has not earned a claim
that it beats Transformer or that CVD is better than random block replacement.

\begin{table}[t]
  \centering
  \caption{Direct structural-dependency preservation on the bundled reference
  snippets.  This table checks the evaluator and the diagnostic definitions; it
  is not model-generation evidence.}
  \label{tab:dependency_preservation}
  \small
  \begin{tabular}{lrrrr}
    \toprule
    Source & Required deps. & Preserved deps. & Preservation & Parse success \\
    \midrule
...
\begin{table}[t]
  \centering
  \caption{Block-size sensitivity on the long structural diagnostic at seed 7,
  hidden size 16, and 8 steps.  Cross-block dependencies are present at every
  tested block size, but this tiny run does not show AST-graph preservation or
  an advantage over the parameter-neighbor Transformer.}
  \label{tab:block_size_sensitivity}
  \small
  \begin{tabular}{rrrrrr}
    \toprule
    $B$ & Def-use & Cross-block & AST loss & Trans. loss & AST / Trans. preservation \\
    \midrule
