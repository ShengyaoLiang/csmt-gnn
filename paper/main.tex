...
\begin{abstract}
I began CSMT-GNN after repeatedly seeing code models produce fluent programs
whose local syntax was correct while the relation that made the code meaningful
had slipped away.  My early notes called this a causal hallucination; in this
draft I use the narrower term structural hallucination, because I learned that
the implementation did not earn a formal causal claim.  CSMT-GNN is my attempt
to give a token language model a small amount of explicit program structure
without turning it into a pure graph model.  It combines block-local Transformer
computation, tokenizer-aligned AST ids, autoregressive block-level graph message
passing, a residual AST gate, and a lightweight regularizer called Contextual
Variable Dropout (CVD).  I also treat the development process as part of the
artifact: the first prototype was produced through multi-LLM assisted coding and
then rewritten after audit, which exposed both the speed and the danger of that
workflow.  This submission is the corrected artifact: a runnable reference
implementation, theoretical checks for information flow and training dynamics,
an explicit cost boundary for the AST side channel, an open release package, and
an evaluation plan for testing whether syntax-aware block states reduce
structural code hallucination.  I do not claim frontier-scale results.  I claim
that the idea and the workflow are now honest enough to be tested.
\end{abstract}
...
This project also contains a correction of my own earlier framing.  I first
...
The artifact I am submitting has five concrete pieces.  First, it defines a
code-modeling architecture that combines block-local attention, trainable AST
embeddings, prefix-masked block graph attention, gated graph-to-token
injection, and top-$k$ mixture-of-experts (MoE) computation.  Second, it ships a
cleaned implementation that removes several failure modes in the earlier
prototype: random non-trainable AST embeddings, token-offset mismatch,
block-size-specific softmax code, distorted CVD sampling, and a Python-loop MoE
path that collapses throughput when specialized kernels are unavailable.  Third,
it gives mathematical checks for prefix information flow, attention-edge count,
AST gate stability, the cost boundary of the syntax channel, and the stochastic
objective optimized by CVD.  Fourth, it documents the multi-LLM assisted
development workflow that produced the first draft and the audit steps that made
the artifact defensible.  Fifth, it gives the evaluation protocol I would trust
before making a stronger claim.

There is also a more personal reason I chose code as the target.  I do not
think the next useful step for AI has to be making models imitate human
consciousness or perform personhood.  I care more about whether models can
become reliable productive instruments.  Code is one of the clearest places
where that matters, because code is not only text; it is a production material
for science, engineering, data analysis, and automation.  A model that writes
fluent but structurally broken code does not merely make a language mistake.  It
damages a tool chain.  That is why I designed CSMT-GNN around program structure
rather than around a broader claim about reasoning in general.

This is also why I used multiple LLMs during the early implementation.  I was
trying to find out whether an independent researcher could move from an
architecture idea to a runnable artifact without institutional-scale support.
The answer was partly yes and partly sobering.  Multi-LLM coding made the first
prototype possible quickly, but it also produced overconfident names, repeated
documentation, fragile kernels, and claims that were stronger than the code.  My
later work was to take authorship back: delete, rename, test, and rewrite until
the artifact said only what it could support.  I include that process because
one claim of this release is methodological: in an AI-assisted research era, the
scarce skill is not merely knowing every implementation detail in advance, but
continuing to think, audit, and take responsibility for the result.
...
also tracks a router load-balancing loss and a router z-loss; for large H100
training, a grouped-GEMM expert kernel remains the right production target.
...
\begin{itemize}
  ...
  paired token and AST NumPy files.
  \item \texttt{scripts/diagnostic\_poc\_train.py} trains an ablation matrix on
  the diagnostic cases when PyTorch is available.
\end{itemize}
...
\subsection{Multi-LLM Assisted Development}

The development path matters for this artifact.  In the early phase, I spent
most of my energy on the architecture: what failure I wanted to attack, where
the AST should enter, what the block graph should communicate, and how to avoid
pretending that a neural mask was a formal causal intervention.  I then used
several LLM systems as coding collaborators to turn those notes into a runnable
prototype quickly.  I used multiple prompt families rather than a single chat
thread because each model tended to expose different blind spots: one was better
at scaffolding, another at kernel-shaped code, another at documentation, and
another at criticizing claims.  This was not a replacement for authorship.  It
was a way to make the search space reachable.

The experience changed my view of AI-assisted research.  A single model often
failed when the task required architecture design, implementation, and
self-critique at the same time.  A coordinated set of models could move faster,
but only because I kept asking what the architecture was supposed to mean.  The
important question was not whether I already knew every implementation detail.
It was whether I kept thinking after the code appeared.  That distinction is
important to me because independent researchers often lack compute, lab
infrastructure, and specialized collaborators.  Multi-LLM development does not
remove those limits, but it can lower the first barrier enough for a serious
idea to become inspectable.

The same process also created errors that I would not defend.  The first
prototype overused causal language, repeated operation guides across files,
contained a block-size-specific softmax path, and used names that sounded more
rigorous than the computation beneath them.  My later work was therefore not
just optimization; it was audit and authorship.  I renamed the unsupported
do-intervention framing to CVD, rewrote AST handling around trainable ids,
removed the brittle kernel from the reference path, added prefix inference
features, corrected batch and padding semantics, added Transformer controls,
and made the release package inspectable.  I include this history because one
contribution of the artifact is the workflow itself: multi-LLM coordination can
help an independent researcher reach a serious prototype, but only if the human
author accepts responsibility for narrowing claims, deleting seductive but false
language, and making the code auditable.
...
I cannot replace training evidence with algebra.  What I can do is rule out some
basic failure modes before spending compute: future leakage in the block graph,
unbounded changes from the AST gate at initialization, fake claims about memory,
and a CVD story that has no objective behind it.  These checks are modest, but
they are the kind of evidence I can afford and the kind a reader can audit.

\begin{proposition}[Prefix visibility]
\label{prop:prefix}
Assume each block-local attention operator uses a causal mask and the block
graph attends only from block $m$ to blocks $n\le m$.  If the AST ids for token
$t$ are computed from a source prefix ending no later than $t$, then the hidden
state used to predict token $t+1$ is a function only of tokens and AST spans in
the prefix $x_{1:t}$.
\end{proposition}
...
blocks with index at most the current block; with the injection shift used in
Section 3.5, block $m$ receives only graph state from blocks $<m$.  Therefore no
later block can affect the current token state.  The remaining possible leakage
...
The replacement vector $v_{\mathrm{cvd}}$ participates in the computation graph
exactly on steps where at least one eligible block is sampled.  If an example has
$D$ eligible blocks, that event has probability
...
trick was the wrong abstraction: the right statement is probabilistic exposure,
not guaranteed participation on every step.  The reference trainer therefore
leaves the optional L2 penalty on $v_{\mathrm{cvd}}$ disabled by default; turning
it on is a practical stabilization choice, not part of the CVD objective above.
...
\begin{table}[t]
  ...
  architecture.}
  \label{tab:stages}
  \begin{tabularx}{\linewidth}{p{0.14\linewidth}p{0.28\linewidth}XX}
    \toprule
...
cuts through syntax, but the right block size still has to be measured.  I like the idea, but
liking an idea is not an argument for deploying it.  If the AST gate and block
graph do not improve downstream reliability enough to justify their cost, the
right conclusion is to simplify the model.
