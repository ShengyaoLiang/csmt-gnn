...
METRICS = ("final_loss", "eval_loss", "definition_use_loss", "cross_block_use_loss")
...
def metric_value(variant: Dict, metric: str) -> float:
    ...
    return float(variant["eval_metrics"][metric])
    ...
    for path in paths:
        ...
        for name, item in by_variant.items():
            ...
            for metric in METRICS:
                rows[name][metric].append(metric_value(item, metric))
...
def markdown_table(summary: Dict) -> str:
    ...
    for row in summary["variants"]:
        metrics = row["metrics"]
        lines.append(
            "| `{variant}` | {num_seeds} | {params} | {final_loss} | {eval_loss} | {cross} |".format(
                variant=row["variant"],
